...
contract DirectMessageProxy is TacProxyV1 {
    uint256 public constant PROTOCOL_FEE = 0.1e18;  // 0.1 wTON (DEMO - JUST FOR ILLUSTRATION)
    uint256 public constant EXECUTOR_FEE = 0.05e18; // 0.05 wTON (DEMO - JUST FOR ILLUSTRATION)
...
contract OneWayProxy is TacProxyV1 {
    uint256 public constant PROTOCOL_FEE = 0.1e18;  // 0.1 wTON
    uint256 public constant EXECUTOR_FEE = 0.05e18; // 0.05 wTON
        ...
        _onlyCrossChainLayer
    {
        ...
        wTON.approve(_getCrossChainLayerAddress(), PROTOCOL_FEE + EXECUTOR_FEE);

        // Send with fees
        OutMessageV1 memory outMsg = OutMessageV1({
            shardsKey: generateNewShardsKey(),     // New operation
            tvmTarget: header.tvmCaller,           // Send to user
            tvmPayload: abi.encode("REWARD_COMPLETED", params.taskId),
            tvmProtocolFee: PROTOCOL_FEE,          // Contract pays protocol fee
            tvmExecutorFee: EXECUTOR_FEE,          // Contract pays executor fee
            tvmValidExecutors: new string[](0),    // Use default executors
            toBridge: rewards,
            toBridgeNFT: new NFTAmount[](0)
        });

        _sendMessageV1(outMsg, PROTOCOL_FEE + EXECUTOR_FEE);
    }
