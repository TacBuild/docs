...
const getSimpleStatusWithWaiting = async (transactionLinker) => {
  ...
  const finalStatus = await tracker.getSimplifiedOperationStatus(
    transactionLinker,
    {
      ...
      successCheck: (status) => status !== "PENDING" && status !== "OPERATION_ID_NOT_FOUND",
      onSuccess: (status) => console.log(`Transaction reached final status: ${status}`)
    }
  );
      ...
      if (context?.enableProfiling && context.operationTracker) {
        ...
        for (const [stageName, stageInfo] of Object.entries(profilingData)) {
          if (stageName !== 'operationType' && stageName !== 'metaInfo') {
            const status = stageInfo.exists ? '✅ Completed' : '⏸️ Not executed';
            const timestamp = stageInfo.timestamp ? new Date(stageInfo.timestamp).toISOString() : 'N/A';
