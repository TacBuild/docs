...
const transactionLinkerWithWait = await tacSdk.sendCrossChainTransaction(
  ...
  sender,
  assets,
  undefined, // options (can be omitted)
  {
    ...
    onSuccess: (result) => console.log("Transaction completed successfully!", result)
  }
);
...
const result = await tacSdk.sendCrossChainTransaction(
  ...
  undefined,
  {
    ...
    onSuccess: (result, context) => {
      console.log(`${context.transactionType} completed for user ${context.userId}`);
    }
  }
);
...
const resultWithProfiling = await tacSdk.sendCrossChainTransaction(
  ...
  undefined,
  {
    ...
    onSuccess: async (operationId, context) => {
      if (context?.enableProfiling && context.operationTracker) {
        ...
        const profilingData = await context.operationTracker.getStageProfiling(operationId);
...
const batchTransactions = async () => {
  ...
  const transactionLinkersWithWait = await tacSdk.sendCrossChainTransactions(
    ...
    crosschainTxs,
    {
      ...
      onSuccess: (results) => console.log(`Batch of ${results.length} transactions completed`)
    }
  );
